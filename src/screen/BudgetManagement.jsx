...
const BudgetManagement = () => {
  ...
  const {
    ...
    requirementOrdered: requirements,
  ...
  const budgetExecution = totalBudget
    ? ((totalRequirementCost / totalBudget) * 100).toFixed(2)
    : 0;
  ...
  return (
    <Container>
      <Text style={styles.header}>
        Ejecutado: {budgetExecution}% de ${totalBudget}
      </Text>
      {totalBudget > 0 ? (
        <View style={styles.progressBarContainer}>
          <View
            style={[styles.progressBar, { width: `${budgetExecution}%` }]}
          />
        </View>
      ) : (
      ...
      <Text style={styles.subHeader}>
        Total de Requerimientos: {totalRequirementCost}$
      </Text>
